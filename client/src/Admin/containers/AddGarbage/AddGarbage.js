...
const AddGarbage = (props) => {
    ...
    let addform = (
        <div className={classes.addform}>
            ...
            {creategarbage.newGarbageQnt==="piece"?<div>
                ...
                <input type="number" placeholder="Enter default weight in lb" value={creategarbage.defaultWeight} onChange={(e) => { setcg({ ...creategarbage,defaultWeight: e.target.value }) }} required />
            </div>:null}
    ...
    let modifyRateform = (
        <div className={classes.addform}>
            ...
            <select className={classes.catg} onChange={(e) =>seteditform({ ...editform, editGarbageRate:e.target.value.split('|')[0],editGarbageName:e.target.value.split('|')[1] })}>
                <option value="sg">Select Garbage</option>
                {
                    props.garbage[editform.subcatagoryNum]?.subcatagory.map(sc => {
                        return (
                            <option  value={`${sc.rate}|${sc.name}`}>{sc.name}</option>
                        )
                    })
                }
            </select>
