...
class Wallet extends Component {
  ...
  onsubmit = async (e) => {
    e.preventDefault();
    await this.props.addadminwallet(parseInt(this.state.inputamount));
    ...
    const wallet = (
      <div className={classes.container}>
        ...
        <div className={classes.amount}>
          <img src={walletimg} className={classes.walletimg} alt="walletimg" />
          <h1>&#x24; {this.props.adminWallet}</h1>
          {!this.state.redeem ? (
            <button
              className={classes.redeembtn}
              onClick={(e) => this.onredeem(e)}
            >
              Add Money
            </button>
          ) : null}
        ...
        <div className={classes.transaction}>
          {this.props.adminTransaction.length === 0 ? (
            <h4 style={{ textAlign: "center" }}>
              You Have Not Made Any Transaction Till Now..
            </h4>
          ) : (
            <div>
              <br />
              <h3 className={classes.heading}>All Transaction Details</h3>
                  ...
                  return (
                    <tr key={transaction._id}>
                      <td style={{ textAlign: "left" }}>
                        ...
                        {transaction.receiver} Garbage Collector
                      </td>
                      <td style={{ color: "red" }}>
                        {transaction.withdraw ? (
                          <span>&#x24; {transaction.withdraw}</span>
                        ) : (
                          "-"
                        )}
                      </td>
