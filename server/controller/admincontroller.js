...
exports.registergc = (req, res) => {
  GarbageCollector.findOne({ city: req.body.city, email: req.body.email })
    .then(async (gc) => {
      if (gc) {
...
exports.cancelgc = async (req, res) => {
  ...
  await GarbageCollector.findByIdAndDelete(gcid)
    .then((resp) => {
...
exports.getwallet = async (req, res) => {
  await wallet
    ...
    .catch((e) => {
      return res.status(400).send("wallet failed.");
    });
};
...
exports.editgarbage = async (req, res) => {
  try {
    await Garbage.findOne({ category: req.body.garbage.category }).then(
      async (resp) => {
        const subname = resp.subcatagory.find(
          (sc) => sc.name === req.body.garbage.subcatagory
        );
...
exports.deletegarbage = async (req, res) => {
  try {
    await Garbage.findOne({ category: req.body.garbage.category }).then(
      async (resp) => {
        resp.subcatagory = resp.subcatagory.filter(
          (sc) => sc.name !== req.body.garbage.subcatagory
        );
        await resp.save();
...
exports.getdata = async (req, res) => {
  try {
    ...
    let totalOrder = 0;
    let totalUser = 0;
    await CompletedOrders.find({})
      .then((order) => {
        for (let i = 0; i < order.length; i++) {
          totalSpend = totalSpend + parseInt(order[i].totalAmount);
          totalGarbageWeight =
            totalGarbageWeight + parseInt(order[i].totalWeight);
        }
      })
      .catch((e) => {
        return res.status(400).send(e);
    ...
    return res
      .status(200)
      .send({ totalSpend, totalGarbageWeight, totalOrder, totalUser });
  } catch (e) {
    return res.status(400).send(e);
  }
};
...
exports.getrecordbycity = async (req, res) => {
  try {
    ...
    if (req.query.city === "allcity") {
      await CompletedOrders.find({})
        .then((orders) => {
          for (let k = 0; k < orders.length; k++) {
            ...
            paperweight += parseInt(
              paperobject[paperobject.length - 1].totalPaperWeight
            );
            paperspend += parseInt(
              paperobject[paperobject.length - 1].totalPaperAmount
            );

            const plasticobject = garbage[1];
            plasticweight += parseInt(
              plasticobject[plasticobject.length - 1].totalPlasticWeight
            );
            plasticspend += parseInt(
              plasticobject[plasticobject.length - 1].totalPlasticAmount
            );

            const metalobject = garbage[2];
            metalweight += parseInt(
              metalobject[metalobject.length - 1].totalMetalWeight
            );
            metalspend += parseInt(
              metalobject[metalobject.length - 1].totalMetalAmount
            );

            const ewasteobject = garbage[3];
            ewasteweight += parseInt(
              ewasteobject[ewasteobject.length - 1].totaleWasteWeight
            );
            ewastespend += parseInt(
              ewasteobject[ewasteobject.length - 1].totaleWasteAmount
            );

            const otherobject = garbage[4];
            otherweight += parseInt(
              otherobject[otherobject.length - 1].totalOtherWeight
            );
            otherspend += parseInt(
              otherobject[otherobject.length - 1].totalOtherAmount
            );
          }
        })
        .catch((e) => {
      ...
      const city =
        req.query.city.charAt(0).toUpperCase() + req.query.city.slice(1);
      await CompletedOrders.find({ city })
        .then((orders) => {
          for (let k = 0; k < orders.length; k++) {
            const garbage = Object.values(orders[k].garbage);
    ...
    return res.status(200).send({
      paper: { paperweight, paperspend },
      plastic: { plasticweight, plasticspend },
      metal: { metalweight, metalspend },
      ewaste: { ewastespend, ewasteweight },
      other: { otherweight, otherspend },
    });
  } catch (e) {
    return res.status(400).send(e);
  }
